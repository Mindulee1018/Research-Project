...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setErr("");
        const [m, d, t, l] = await Promise.all([
  ...
  const driftChartData = useMemo(() => {
    return drift.map((r) => ({
      ...
      concept:
        r.concept_mean_abs_delta === "" ? null : Number(r.concept_mean_abs_delta),
    ...
    <div className="container">
      ...
      <div className="grid" style={{ gridTemplateColumns: "1.4fr 1fr", gap: 12 }}>
        ...
        <div className="card">
          ...
          <div className="small">
            JSD=data drift (term distribution). Concept=relationship drift proxy.
            Trigger line shows incremental update points.
          </div>
        </div>
          ...
          {!latestTrigger ? (
            ...
            <div>
              <div style={{ marginBottom: 8 }}>
                ...
                <span className="pill">Vote count: {latestTrigger.vote_count}</span>
...
{JSON.stringify(latestTrigger.votes, null, 2)}
        ...
        {/* New terms over time */}
        <div className="card" style={{ gridColumn: "1 / -1" }}>
          <div className="sectionTitle">New Terms Over Time (by Trigger Batch)</div>
            ...
            {[...triggers].reverse().map((t, idx) => (
              <div className="listItem" key={idx}>
                ...
                <div className="small" style={{ marginTop: 4 }}>
                  {(t.new_terms || []).length ? t.new_terms.join(", ") : "No new terms"}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card" style={{ gridColumn: "1 / -1" }}>
          ...
          <div className="grid" style={{ gridTemplateColumns: "1fr", gap: 10 }}>
            {[...triggers].reverse().map((t, idx) => (
              <div className="listItem" key={idx}>
                <div style={{ fontWeight: 700 }}>
                  {t.batch_no} <span className="small">— votes: {t.vote_count}</span>
                </div>
                <div className="small">
                ...
                <div className="small">
                  JSD: {t.jsd ?? "-"} | new_terms_in_hate: {t.new_terms_in_hate ?? "-"}
                </div>
                <div className="small" style={{ marginTop: 4 }}>
