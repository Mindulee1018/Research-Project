...
class ElakiriAdapter(SourceAdapter):
    source_name = "elakiri"
    ...
    def load(self, config: Dict[str, str]) -> List[SourceRecord]:
        ...
        if urls:
            ...
            if crawl_enabled:
                ...
                domains = config.get("allowed_domains", ["elakiri.com"])
                ...
                for page in pages:
                    ...
                    comments = extract_comment_candidates_from_html(
                        ...
                        keywords=config.get(
                            "comment_keywords",
                            ["message-body", "bbwrapper", "comment", "reply"],
                        ),
                    )
            ...
            for idx, url in enumerate(urls, start=1):
                ...
                records.append(
                    SourceRecord(
                        source_item_id_or_url=url or f"elakiri_url_{idx}",
                        raw_text=text,
        ...
        if not input_csv and not input_json:
            raise ValueError(
                "Elakiri adapter requires URL mode ('urls'/'urls_file') "
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            source_item = resolve_source_item(row, fallback_prefix="elakiri_item", index=idx)
