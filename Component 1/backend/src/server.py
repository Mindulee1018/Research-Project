...
async def process(req: ProcessRequest):
    # ── Check duplicate video (skip if already processed) ───────
...
# ── Download comments CSV by Video ID ─────────────────────────
...
async def run_pipeline(job_id, youtube_url, max_comments):
    job = jobs[job_id]
    try:
        # ── Generate Video ID from MongoDB ───────────────────────
        ...
        if comment_results:
            try:
                ...
                print(f" Saved {len(comment_results)} comments to MongoDB")
            except Exception as e:
                print(f"⚠️ Some comments may be duplicates: {e}")
        
        try:
            posts_col.insert_one(post_result)
            print(f" Saved Video ID {video_id} to MongoDB!")
        except DuplicateKeyError:
            print(f"⚠️ Post already exists for {youtube_url}")
        ...
        # ── Auto-save CSVs to Component 1 data/batches (backup) ───
        ...
        try:
            ...
            print(f" CSVs backed up in Component 1 batches: Video ID {video_id}")
        except Exception as e:
            print(f"⚠️ Could not save to Component 1 batches: {e}")

        # ── Auto-save CSVs to Component 2 data folder (backup) ────
        ...
        try:
            ...
            print(f" CSVs backed up to Component 2: Video ID {video_id}")
        except Exception as e:
            print(f"⚠️ Could not save to Component 2: {e}")
